...
def train():
    ...
    CUBE_BATCH = 1024  # Pass 1024 image cube pairs to model.fit (~5.3 GB)
    ...
    input_data = np.ndarray((CUBE_BATCH, *CUBE_IMAGE_SHAPE))
    output_data = np.ndarray((CUBE_BATCH, *CUBE_IMAGE_SHAPE))
    ...
    for batch_index in range(0, len(inputs), CUBE_BATCH):
        for index, (in_file, out_file) in enumerate(
            zip(inputs[batch_index:batch_index + CUBE_BATCH - 1], outputs[batch_index:batch_index + CUBE_BATCH - 1])):
